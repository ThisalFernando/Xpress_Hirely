...
const Payments_AdminView = () => {
  ...
  useEffect(() => {
    const filteredData = cardpayments.filter((item) => {
      return (
        item.CardHolderName.toLowerCase().includes(searchInput.toLowerCase()) ||
  ...
  useEffect(() => {
    const filteredData = cashpayments.filter((item) => {
      return (
        item.ReceiptNo.toString().includes(searchInput) ||
        item.Amount.toString().includes(searchInput) ||
  ...
  useEffect(() => {
    const filteredData = refundrequests.filter((item) => {
      return (
        item.BookingID.toString().includes(searchInput) ||
  ...
  return (
    <div
      className="flex flex-col h-screen justify-center items-center h-screen  bg-cover bg-center"
      style={{ backgroundImage: `url(${adminbg})` }}
    >
      <div className="mt-10 mb-10">
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("AllCardPaymentsTable")}
        >
          Card
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("AllCashPaymentsTable")}
        >
          Cash
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("AllRefundRequestsTable")}
        >
          Refund
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("AllStripePaymentsTable")}
        >
          Stripe
        </button>
        <br />
      ...
      {loading ? (
        <Spinner />
      ) : showType === "AllCardPaymentsTable" ? (
        <AllCardPaymentsTable cardpayments={filteredCardPayments} />
      ) : showType === "AllCashPaymentsTable" ? (
        <AllCashPaymentsTable cashpayments={filteredCashPayments} />
      ) : showType === "AllRefundRequestsTable" ? (
        <AllRefundRequestsTable refundrequests={filteredRefundRequests} />
      ) : showType === "AllStripePaymentsTable" ? (
        <AllStripePaymentsTable stripepayments={filteredStripePayments} />
      ) : null}
    </div>
  );
};
