    ...
    >
      <div>
        <button
          ...
          onClick={() => setShowType("CardPaymentsTable")}
        >
          Card
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("CashPaymentsTable")}
        >
          Cash
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("StripePaymentsTable")}
        >
          Stripe
        </button>
        &ensp;&ensp;
        <button
          className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg"
          onClick={() => setShowType("PaymentMethodTable")}
        >
          PaymentMethod
        </button>
      </div>
      ...
      {loading ? (
        <Spinner />
      ) : showType === "CardPaymentsTable" ? (
        <CardPaymentsTable cardpayments={filteredCardPayments} />
      ) : showType === "CashPaymentsTable" ? (
        <CashPaymentsTable cashpayments={filteredCashPayments} />
      ) : showType === "PaymentMethodTable" ? (
        <PaymentMethodTable paymentmethods={paymentmethods} />
      ) : showType === "StripePaymentsTable" ? (
        <StripePaymentsTable stripepayments={filteredStripePayments} />
      ) : null}
      <br />

      <div>
        <Link to="/refundrequests/user/create">
          <button className="bg-white text-black border border-orange-500 hover:bg-orange-500 hover:text-black focus:bg-orange-500 focus:text-black active:bg-orange-500 active:text-black px-4 py-2 rounded-lg">
